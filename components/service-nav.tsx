...
export function ServiceNav() {
  ...
  return (
    <div className="border-b border-gray-100 bg-[#fffbf6] py-8">
      <div className="container relative">
        {/* Scrollable strip */}
        <div
          ref={scrollRef}
          className="flex items-center justify-between gap-8 overflow-x-auto pb-4 scrollbar-hide"
        >
          {services.map((service) => (
            <Link
              key={service.label}
              href={service.href}
              className="group flex min-w-[100px] flex-col items-center gap-4 text-center"
            >
              <div className="flex h-12 w-12 items-center justify-center text-[#4b3624] transition-transform duration-300 group-hover:-translate-y-1">
                <service.icon className="h-8 w-8" strokeWidth={1.5} />
              </div>
              <div className="flex flex-col items-center gap-2">
                <span className="text-[13px] font-medium text-[#4b3624]">
                  {service.label}
                </span>
                {service.active && <div className="h-1 w-12 bg-[#d4a574]" />}
              </div>
            </Link>
          ))}
        </div>
        ...
        >
          <ChevronLeft className="h-4 w-4 t text-[#4b3624]" />
        </button>
